...
> 강의일: 2026-09-18 · 사용자 지정 강의일 기준으로 자료를 정리한 학습 노트
> 확인 범위: `정규식_넘파이.pdf` 물리 1–99쪽 전체. 한국어·영어 OCR로 읽고 핵심 패턴·코드·shape 예제를 원본 이미지와 대조했다.
...
`match`는 시작 위치를 제한할 뿐 끝까지 소비한다고 보장하지 않는다. 형식 전체 검증에는 `fullmatch`가 명확하다. `finditer`는 결과 리스트를 한 번에 만들지 않지만, 이미 입력으로 받은 전체 문자열의 메모리까지 없애는 것은 아니다. [강의: 10–15쪽, 검증 의미 보완]
...
Python 문자열 해석과 정규식 해석은 서로 다른 단계다. `r"\d+"`처럼 raw string을 쓰면 Python 문자열 단계의 백슬래시 이스케이프와 혼동을 줄일 수 있다. 정규식 엔진은 여전히 `\d`를 숫자 패턴으로 해석한다. [강의: 17–24쪽]
...
역참조는 패턴을 단순 재사용하는 것이 아니다. 예를 들어 두 단어의 형식이 같기만 한 경우와 두 단어의 내용까지 같은 경우를 구별한다. [강의: 26–33, 39쪽, 역참조 의미 교정]
...
수량자는 기본적으로 가능한 많이 소비한다. `.*?`처럼 `?`를 붙이면 전체 패턴을 만족하는 범위에서 적게 소비하려 한다. “항상 가장 짧은 문자열만 찾는다”보다 **나머지 패턴의 성공까지 함께 고려한다**고 이해하는 편이 정확하다. [강의: 36–42쪽]
...
`ndim`은 축 개수, `shape`은 각 축의 길이, `dtype`은 원소 저장 타입이다. 배열은 동일한 dtype으로 원소를 다루므로 Python 객체별 연산 비용을 줄일 수 있다. 숫자 배열에서 큰 장점이 있지만 `object` dtype이나 아주 작은 배열까지 항상 빠르다고 일반화하지 않는다. [강의: 45–61쪽]
...
벡터화는 Python에서 원소마다 반복하는 대신 배열 연산을 호출하는 방식이다. 반복 자체가 없어지는 것은 아니고 NumPy 내부에서 수행된다. 브로드캐스팅은 모양이 다른 배열의 축을 맞춰 연산하는 규칙이다. [강의: 60–67쪽]
...
집계에서 axis는 줄여 없앨 축이다. `(3,4)`의 `sum(axis=0)`은 첫 축 길이 3을 줄여 `(4,)`, `sum(axis=1)`은 두 번째 축 길이 4를 줄여 `(3,)`가 된다. `keepdims=True`는 줄인 축을 길이 1로 남겨 원래 배열과 다시 연산하기 쉽게 한다. [강의: 53–54, 70–71쪽]
...
`std`는 기본 `ddof=0`이며 표본 표준편차가 목적이면 정의에 맞춰 설정한다. `nanmean` 같은 함수는 NaN을 제외하지만 전부 결측인 경우 등은 별도 처리가 필요하다. 고정 폭 정수의 오버플로우는 dtype 선택으로 관리한다. `reshape`가 항상 뷰라는 보장도 없다. [강의: 69–83쪽, 조건 보완]
...
기본 슬라이스는 원본 메모리를 공유한다. 정수 배열·불리언 마스크로 고급 인덱싱해서 얻은 배열은 복사본이다. 단, `a[mask] = value`처럼 **원본을 대입 대상으로 쓰면 원본을 수정**한다. 읽기 결과가 복사라는 말과 혼동하지 않는다. [강의: 56–58, 85–96쪽]
...
한 줄의 일부 숫자만 조용히 추출하지 않도록 전체 검증을 선택했다. 유효 데이터가 없을 때 평균을 계산하지 않고 None으로 처리한다. 누락이 중요한 분석이라면 오류 행을 버리는 대신 센서 ID와 결측 상태를 함께 보존해야 한다. [강의: 8쪽의 추출→배열 흐름을 바탕으로 별도 예제 작성]

## 10. 교정·검증과 복습

- 10쪽 `match`의 엄격한 검증 용도에는 문자열 끝 확인이 추가로 필요하다. 본문은 `fullmatch`로 구분했다.
- 17쪽 문자 클래스 표의 ASCII처럼 읽히는 설명은 기본 Unicode 의미로 보완했다. 23쪽은 이미 이 차이를 언급한다.
- 33쪽 역참조는 패턴 재사용이 아니라 앞서 일치한 문자열의 재사용이다.
- 37쪽 `\d+(?!개)`는 숫자 일부를 매치할 수 있다. 위 반례를 실행해 확인했다.
- 50–61쪽의 연속 메모리 설명은 일반적인 수치 배열의 이점이다. 전치·슬라이스 뷰는 비연속일 수 있다.
- 71쪽 `keepdims`는 브로드캐스팅을 편하게 하지만 수동 차원 추가로 대체할 수 있어 필수 문법은 아니다.
- 82쪽 안정 정렬 옵션은 안정성을 요청하는 의미로 이해한다. `mergesort`와 `stable`의 내부 구현이 모든 dtype에서 동일하다고 보장하지 않는다.
